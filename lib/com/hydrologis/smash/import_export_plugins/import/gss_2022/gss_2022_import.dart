...
class Gss2022ImportPlugin extends AImportPlugin {
  ...
  Widget getImportPage() {
    return const Gss2022ImportWidget();
  }

  @override
  Widget getSettingsPage() {
    return Gss2022Settings();
  }
}

class Gss2022ImportWidget extends StatefulWidget {
  const Gss2022ImportWidget({Key? key}) : super(key: key);

  @override
  _Gss2022ImportWidgetState createState() => _Gss2022ImportWidgetState();
}

class _Gss2022ImportWidgetState extends State<Gss2022ImportWidget>
  ...
  Future<void> init() async {
    ...
    String downloadDataListUrl =
        _serverUrl! + Gss2022Utilities.DATA_DOWNLOAD_PATH;
    String downloadTagsListUrl =
        _serverUrl! + Gss2022Utilities.TAGS_DOWNLOAD_PATH;
    ...
    _authHeader = await Gss2022Utilities.getAuthHeader(pwd);
    ...
    try {
      ...
      List<dynamic> baseMaps =
          dataResponseMap[Gss2022Utilities.DATA_DOWNLOAD_MAPS];
      _baseMapsList.clear();
      for (var bm in baseMaps) {
        var name = bm[Gss2022Utilities.DATA_DOWNLOAD_NAME];
      ...
      List<dynamic> _projects =
          dataResponseMap[Gss2022Utilities.DATA_DOWNLOAD_PROJECTS];
      _projectsList.clear();
      for (var proj in _projects) {
        var name = proj[Gss2022Utilities.DATA_DOWNLOAD_NAME];
      ...
      var tagsJsonList = tagsResponseMap[Gss2022Utilities.TAGS_DOWNLOAD_TAGS];
      if (tagsJsonList != null) {
        tagsJsonList.forEach((tg) {
          var name = tg[Gss2022Utilities.TAGS_DOWNLOAD_TAG];
          _tagsList.add(name);
        });
      }
